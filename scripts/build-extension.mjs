...
export async function buildExtension(out = "out") {
    ...
    const outdir = path.join(out, extensionId + ".bike-extension");
    const prod = (process.argv[2] === "production");
    const context = await esbuild.context({
        banner: { js: "// GENERATED BY ESBUILD" },
        logOverride: { "empty-glob": "silent" },
        entryPoints: [
            // app context entry points (0-1)
            "src/app/main.ts",
            // style context entry points (0-1)
            "src/style/main.ts",
            // dom context entry points (0-n ts/tsx)
            "src/dom/*-dom-script.ts",
            "src/dom/*-dom-script.tsx",
        ],
        external: [
            "react",
            "react-dom",
